...
**1 Jul 2026 - 20 Aug 2026** (37 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-5%2C869-green) ![stocks](https://img.shields.io/badge/stocks-882-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 5,869 (from 7,590 raw rows) |
| Stocks | 882 |
| Institutions | 1,776 |
| Buy value | Rs 66,968.85 cr |
| Sell value | Rs 82,402.24 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          5,869 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 882
stocks, 1,776 institutions, 37 days and 2,822 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 5,869 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   807 pairs, 1,614 rows. This is overwhelmingly intraday market-maker and arbitrage
   churn (NK Securities, Junomoneta Finsol, HRTI, Microcurves and similar).
2. **Feed duplicates removed.** 107 rows where one trade was reported twice, once in
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 806 of 5,869 rows and turnover falls from Rs 1,49,371.10 cr to Rs 1,22,192.24 cr; at 0.1% it hides 304 rows whose combined residual net position is only about Rs 6.17 cr.
...
| | |
|---|---|
| Rows kept | 2,793 of 5,869 |
| Rows removed | 3,076, across 1,474 institution + stock + day combinations |
| Stocks | 714 |
| Institutions | 1,623 |
| Buy value | Rs 38,610.29 cr |
| Sell value | Rs 54,929.42 cr |
| Turnover | Rs 93,539.71 cr |
...
shows the same 2,793 rows whether the round-trip toggle is on or off. Note that it
...
- `data.json` holds the cleaned 5,869 rows only. The 1,721 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~1368 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~1368 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 1331 KB | The 5,869 cleaned deal records. The only stored data artefact. |
