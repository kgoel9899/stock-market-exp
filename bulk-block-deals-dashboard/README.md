...
**1 Jul 2026 - 21 Aug 2026** (38 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-6%2C042-green) ![stocks](https://img.shields.io/badge/stocks-903-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 6,042 (from 7,814 raw rows) |
| Stocks | 903 |
| Institutions | 1,829 |
| Buy value | Rs 68,422.85 cr |
| Sell value | Rs 84,098.59 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          6,042 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 903
stocks, 1,829 institutions, 38 days and 2,917 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 6,042 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   826 pairs, 1,652 rows. This is overwhelmingly intraday market-maker and arbitrage
   churn (NK Securities, Junomoneta Finsol, HRTI, Microcurves and similar).
2. **Feed duplicates removed.** 120 rows where one trade was reported twice, once in
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 822 of 6,042 rows and turnover falls from Rs 1,52,521.44 cr to Rs 1,24,927.75 cr; at 0.1% it hides 310 rows whose combined residual net position is only about Rs 6.22 cr.
...
| | |
|---|---|
| Rows kept | 2,892 of 6,042 |
| Rows removed | 3,150, across 1,510 institution + stock + day combinations |
| Stocks | 730 |
| Institutions | 1,676 |
| Buy value | Rs 39,643.66 cr |
| Sell value | Rs 56,226.35 cr |
| Turnover | Rs 95,870.01 cr |
...
shows the same 2,892 rows whether the round-trip toggle is on or off. Note that it
...
- `data.json` holds the cleaned 6,042 rows only. The 1,772 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~1407 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~1407 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 1370 KB | The 6,042 cleaned deal records. The only stored data artefact. |
