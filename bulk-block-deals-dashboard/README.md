...
**1 Jul 2026 - 11 Sep 2026** (53 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-9%2C380-green) ![stocks](https://img.shields.io/badge/stocks-1%2C189-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 9,380 (from 12,078 raw rows) |
| Stocks | 1,189 |
| Institutions | 2,552 |
| Buy value | Rs 1,44,895.01 cr |
| Sell value | Rs 1,71,411.83 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          9,380 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 1,189
stocks, 2,552 institutions, 53 days and 4,381 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 9,380 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   1,237 pairs, 2,474 rows. This is overwhelmingly intraday market-maker and arbitrage
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 1,433 of 9,380 rows and turnover falls from Rs 3,16,306.84 cr to Rs 2,70,860.74 cr; at 0.1% it hides 579 rows whose combined residual net position is only about Rs 9.38 cr.
...
| | |
|---|---|
| Rows kept | 4,482 of 9,380 |
| Rows removed | 4,898, across 2,329 institution + stock + day combinations |
| Stocks | 967 |
| Institutions | 2,371 |
| Buy value | Rs 94,440.00 cr |
| Sell value | Rs 1,17,021.43 cr |
| Turnover | Rs 2,11,461.43 cr |
...
- `data.json` holds the cleaned 9,380 rows only. The 2,698 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~2168 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~2168 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 2131 KB | The 9,380 cleaned deal records. The only stored data artefact. |
