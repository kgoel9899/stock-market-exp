...
**1 Jul 2026 - 27 Aug 2026** (42 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-6%2C956-green) ![stocks](https://img.shields.io/badge/stocks-973-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 6,956 (from 8,979 raw rows) |
| Stocks | 973 |
| Institutions | 2,048 |
| Buy value | Rs 82,979.62 cr |
| Sell value | Rs 1,12,407.38 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          6,956 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 973
stocks, 2,048 institutions, 42 days and 3,349 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 6,956 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   931 pairs, 1,862 rows. This is overwhelmingly intraday market-maker and arbitrage
   churn (NK Securities, Junomoneta Finsol, HRTI, Microcurves and similar).
2. **Feed duplicates removed.** 161 rows where one trade was reported twice, once in
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 997 of 6,956 rows and turnover falls from Rs 1,95,387.01 cr to Rs 1,64,749.36 cr; at 0.1% it hides 419 rows whose combined residual net position is only about Rs 7.10 cr.
...
| | |
|---|---|
| Rows kept | 3,345 of 6,956 |
| Rows removed | 3,611, across 1,712 institution + stock + day combinations |
| Stocks | 795 |
| Institutions | 1,888 |
| Buy value | Rs 50,845.00 cr |
| Sell value | Rs 81,311.07 cr |
| Turnover | Rs 1,32,156.07 cr |
...
shows the same 3,345 rows whether the round-trip toggle is on or off. Note that it
...
- `data.json` holds the cleaned 6,956 rows only. The 2,023 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~1616 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~1616 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 1579 KB | The 6,956 cleaned deal records. The only stored data artefact. |
