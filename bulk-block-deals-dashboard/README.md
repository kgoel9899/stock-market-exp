...
**1 Jul 2026 - 19 Aug 2026** (36 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-5%2C684-green) ![stocks](https://img.shields.io/badge/stocks-859-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 5,684 (from 7,351 raw rows) |
| Stocks | 859 |
| Institutions | 1,727 |
| Buy value | Rs 64,367.07 cr |
| Sell value | Rs 79,926.50 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          5,684 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 859
stocks, 1,727 institutions, 36 days and 2,737 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 5,684 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   785 pairs, 1,570 rows. This is overwhelmingly intraday market-maker and arbitrage
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 770 of 5,684 rows and turnover falls from Rs 1,44,293.58 cr to Rs 1,17,824.24 cr; at 0.1% it hides 298 rows whose combined residual net position is only about Rs 6.13 cr.
...
| | |
|---|---|
| Rows kept | 2,708 of 5,684 |
| Rows removed | 2,976, across 1,425 institution + stock + day combinations |
| Stocks | 698 |
| Institutions | 1,579 |
| Buy value | Rs 36,794.39 cr |
| Sell value | Rs 53,240.07 cr |
| Turnover | Rs 90,034.46 cr |
...
shows the same 2,708 rows whether the round-trip toggle is on or off. Note that it
...
- `data.json` holds the cleaned 5,684 rows only. The 1,667 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~1326 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~1326 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 1289 KB | The 5,684 cleaned deal records. The only stored data artefact. |
