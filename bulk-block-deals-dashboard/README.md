...
**1 Jul 2026 - 28 Aug 2026** (43 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-7%2C193-green) ![stocks](https://img.shields.io/badge/stocks-997-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 7,193 (from 9,290 raw rows) |
| Stocks | 997 |
| Institutions | 2,095 |
| Buy value | Rs 88,379.92 cr |
| Sell value | Rs 1,17,735.81 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          7,193 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 997
stocks, 2,095 institutions, 43 days and 3,451 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 7,193 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   963 pairs, 1,926 rows. This is overwhelmingly intraday market-maker and arbitrage
   churn (NK Securities, Junomoneta Finsol, HRTI, Microcurves and similar).
2. **Feed duplicates removed.** 171 rows where one trade was reported twice, once in
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 1,035 of 7,193 rows and turnover falls from Rs 2,06,115.72 cr to Rs 1,74,203.15 cr; at 0.1% it hides 441 rows whose combined residual net position is only about Rs 7.25 cr.
...
| | |
|---|---|
| Rows kept | 3,471 of 7,193 |
| Rows removed | 3,722, across 1,762 institution + stock + day combinations |
| Stocks | 814 |
| Institutions | 1,931 |
| Buy value | Rs 54,884.51 cr |
| Sell value | Rs 85,279.96 cr |
| Turnover | Rs 1,40,164.47 cr |
...
shows the same 3,471 rows whether the round-trip toggle is on or off. Note that it
...
- `data.json` holds the cleaned 7,193 rows only. The 2,097 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~1670 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~1670 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 1633 KB | The 7,193 cleaned deal records. The only stored data artefact. |
