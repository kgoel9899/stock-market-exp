...
**1 Jul 2026 - 12 Aug 2026** (31 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-4%2C752-green) ![stocks](https://img.shields.io/badge/stocks-749-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 4,752 (from 6,183 raw rows) |
| Stocks | 749 |
| Institutions | 1,494 |
| Buy value | Rs 49,892.04 cr |
| Sell value | Rs 56,937.16 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          4,752 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 749
stocks, 1,494 institutions, 31 days and 2,320 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 4,752 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   680 pairs, 1,360 rows. This is overwhelmingly intraday market-maker and arbitrage
   churn (NK Securities, Junomoneta Finsol, HRTI, Microcurves and similar).
2. **Feed duplicates removed.** 71 rows where one trade was reported twice, once in
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 678 of 4,752 rows and turnover falls from Rs 1,06,829.20 cr to Rs 82,136.75 cr; at 0.1% it hides 262 rows whose combined residual net position is only about Rs 2.49 cr.
...
| | |
|---|---|
| Rows kept | 2,226 of 4,752 |
| Rows removed | 2,526, across 1,211 institution + stock + day combinations |
| Stocks | 609 |
| Institutions | 1,357 |
| Buy value | Rs 25,617.86 cr |
| Sell value | Rs 33,200.31 cr |
| Turnover | Rs 58,818.17 cr |
...
shows the same 2,226 rows whether the round-trip toggle is on or off. Note that it
...
```js
const raw    = await scrape('2026-08-13', '2026-08-19');  // new range only
...
- `data.json` holds the cleaned 4,752 rows only. The 1,431 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~1114 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~1114 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 1077 KB | The 4,752 cleaned deal records. The only stored data artefact. |
