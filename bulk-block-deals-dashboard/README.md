...
**1 Jul 2026 - 8 Sep 2026** (50 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-8%2C737-green) ![stocks](https://img.shields.io/badge/stocks-1%2C144-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 8,737 (from 11,271 raw rows) |
| Stocks | 1,144 |
| Institutions | 2,428 |
| Buy value | Rs 1,36,352.10 cr |
| Sell value | Rs 1,62,058.97 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          8,737 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 1,144
stocks, 2,428 institutions, 50 days and 4,117 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 8,737 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   1,167 pairs, 2,334 rows. This is overwhelmingly intraday market-maker and arbitrage
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 1,331 of 8,737 rows and turnover falls from Rs 2,98,411.07 cr to Rs 2,56,910.05 cr; at 0.1% it hides 553 rows whose combined residual net position is only about Rs 8.75 cr.
...
| | |
|---|---|
| Rows kept | 4,168 of 8,737 |
| Rows removed | 4,569, across 2,169 institution + stock + day combinations |
| Stocks | 933 |
| Institutions | 2,246 |
| Buy value | Rs 90,545.41 cr |
| Sell value | Rs 1,12,361.23 cr |
| Turnover | Rs 2,02,906.64 cr |
...
- `data.json` holds the cleaned 8,737 rows only. The 2,534 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~2022 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~2022 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 1985 KB | The 8,737 cleaned deal records. The only stored data artefact. |
