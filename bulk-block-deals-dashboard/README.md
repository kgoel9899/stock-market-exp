...
**1 Jul 2026 - 26 Aug 2026** (41 trading days).

![tabs](https://img.shields.io/badge/tabs-9-blue) ![deals](https://img.shields.io/badge/deals-6%2C742-green) ![stocks](https://img.shields.io/badge/stocks-955-orange)

## What it does

| | |
|---|---|
| Qualifying deals | 6,742 (from 8,695 raw rows) |
| Stocks | 955 |
| Institutions | 2,000 |
| Buy value | Rs 79,834.15 cr |
| Sell value | Rs 1,05,916.58 cr |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          6,742 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
its underlying deals without re-scanning. For the current dataset that yields 955
stocks, 2,000 institutions, 41 days and 3,264 stock+institution pairs. Being pure, it
...
aggregate and every headline number depends on the filter - at 6,742 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   903 pairs, 1,806 rows. This is overwhelmingly intraday market-maker and arbitrage
   churn (NK Securities, Junomoneta Finsol, HRTI, Microcurves and similar).
2. **Feed duplicates removed.** 147 rows where one trade was reported twice, once in
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 950 of 6,742 rows and turnover falls from Rs 1,85,750.73 cr to Rs 1,56,259.74 cr; at 0.1% it hides 388 rows whose combined residual net position is only about Rs 6.69 cr.
...
| | |
|---|---|
| Rows kept | 3,254 of 6,742 |
| Rows removed | 3,488, across 1,659 institution + stock + day combinations |
| Stocks | 778 |
| Institutions | 1,844 |
| Buy value | Rs 48,794.15 cr |
| Sell value | Rs 75,860.31 cr |
| Turnover | Rs 1,24,654.47 cr |
...
shows the same 3,254 rows whether the round-trip toggle is on or off. Note that it
...
- `data.json` holds the cleaned 6,742 rows only. The 1,953 removed rows are not
...
| File | Size | Role |
|---|---|---|
| `index.html` | ~1567 KB | **The dashboard.** Self-contained single file with `app.js` and the data inlined. This is what GitHub Pages serves and what you open locally. |
| `dashboard.html` | ~1567 KB | Byte-identical copy of `index.html`, kept under its original name. |
...
| `data.json` | 1530 KB | The 6,742 cleaned deal records. The only stored data artefact. |
