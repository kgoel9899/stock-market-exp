...
| | |
|---|---|
| Qualifying deals | 4,593 (from 5,980 raw rows) |
| Stocks | 725 |
| Institutions | 1,440 |
...
```
  ...
  scrape.js          parse #bbdealTable -> raw rows -> clean()
      |
      v
  data.json          4,593 deal records - the only stored artefact
      |
      +-------------------------+
      |                         |
      v                         v
...
aggregate and every headline number depends on the filter - at 4,593 rows it is
...
1. **Matched round trips removed.** Any client that both bought *and* sold the same
   ...
   658 pairs, 1,316 rows. This is overwhelmingly intraday market-maker and arbitrage
...
same day, a gap of 65 shares on 8.8 million. At 1% it hides 664 of 4,593 rows and turnover falls from Rs 98,313.22 cr to Rs 74,318.88 cr; at 0.1% it hides 252 rows whose combined residual net position is only about Rs 5.11 cr.
...
| | |
|---|---|
| Rows kept | 2,138 of 4,593 |
| Rows removed | 2,455, across 1,178 institution + stock + day combinations |
| Stocks | 589 |
| Institutions | 1,304 |
| Buy value | Rs 23,828.24 cr |
| Sell value | Rs 27,871.87 cr |
| Turnover | Rs 51,700.11 cr |
...
shows the same 2,138 rows whether the round-trip toggle is on or off. Note that it
...
```js
const raw    = await scrape('2026-08-12', '2026-08-18');  // new range only
...
- `data.json` holds the cleaned 4,593 rows only. The 1,387 removed rows are not
