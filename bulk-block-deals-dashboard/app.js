...
function renderDashboard(DEALS, meta) {
  const S = window.__RTS = window.__RTS || { on: false, tol: 1, tab: null };
  ...
  paginatedTable('mDay', {
    ...
    expand: r => {
      ...
      setTimeout(() => {
        ...
        m.dataset.done = 1;
        paginatedTable(m, {
          rows: orderDeals(rbd[r.name] || []), pageSize: 25, sizes: [10, 25, 50, 100],
  ...
  paginatedTable('mDeals', {
    rows: orderDeals(DEALS), sortIdx: 0, pageSize: 50, placeholder: 'Search stock, institution, date, exchange...',
