...
function paginatedTable(mount, o) {
  ...
  let size = o.pageSize || 250, page = 1, q = '', sortI = (o.sortIdx != null ? o.sortIdx : null), dir = -1, view = o.rows.slice();
...
// Canonical ordering for any list of deals:
//   newest day first -> biggest client+stock group that day -> buy leg before sell leg -> larger value.
function orderDeals(rows) {
  const key = r => r.day + '\u0000' + r.client + '\u0000' + r.stock;
  const gt = {};
  rows.forEach(r => { const k = key(r); gt[k] = (gt[k] || 0) + r.val; });
  return rows.slice().sort((a, b) => {
    if (a.day !== b.day) return a.day < b.day ? 1 : -1;
    const ka = key(a), kb = key(b);
    if (ka !== kb) { const d = gt[kb] - gt[ka]; return d !== 0 ? d : (ka < kb ? -1 : 1); }
    const ra = a.action === 'Purchase' ? 0 : 1, rb = b.action === 'Purchase' ? 0 : 1;
    if (ra !== rb) return ra - rb;
    return b.val - a.val;
  });
}

// Drop every (day, client, stock) group whose buy and sell quantities agree within tolPct percent.
function excludeRoundTrips(rows, tolPct) {
  const key = r => r.day + '\u0000' + r.client + '\u0000' + r.stock;
  const g = {};
  rows.forEach(r => {
    const k = key(r), o = g[k] || (g[k] = { bq: 0, sq: 0 });
    if (r.action === 'Purchase') o.bq += r.q; else o.sq += r.q;
  });
  const drop = {};
  Object.keys(g).forEach(k => {
    const o = g[k];
    if (o.bq > 0 && o.sq > 0 && Math.abs(o.bq - o.sq) / Math.max(o.bq, o.sq) * 100 <= tolPct) drop[k] = 1;
  });
  return rows.filter(r => !drop[key(r)]);
}

function rerenderDashboard() { const S = window.__RTS; renderDashboard(S.all, S.meta); }
...
// The tab the dashboard opens on. 'td' is the Latest single-day view.
const DEFAULT_TAB = 'td';

function renderDashboard(DEALS, meta) {
  const S = window.__RTS = window.__RTS || { on: true, tol: 1, tab: DEFAULT_TAB };
  ...
  // The latest date present in the data, taken from the unfiltered array so the
  // round-trip toggle can never move it. Deliberately not the machine's clock: past
  // midnight, or before a scrape has run, the clock is ahead of the data.
  const LATEST = S.all.reduce((m, r) => r.day > m ? r.day : m, '');
  const TABS = [['ov', 'Overview'], ['td', 'Latest \u00B7 ' + (LATEST ? dfmt(LATEST) : 'no data')], ['st', 'By Stock'], ['in', 'By Institution'], ['gs', 'Stock \u2192 Institutions'], ['gi', 'Institution \u2192 Stocks'], ['dy', 'Daily'], ['dc', 'Daily \u00B7 One-Sided'], ['dl', 'All Deals']];
  const OPEN_TAB = TABS.some(t => t[0] === S.tab) ? S.tab : DEFAULT_TAB;
  ...
  document.body.innerHTML = '<div class="wrap">' +
    ...
    '<div class="note" style="display:flex;align-items:center;gap:11px;flex-wrap:wrap">' +
      '<label style="display:flex;align-items:center;gap:7px;cursor:pointer;user-select:none">' +
        '<input type="checkbox" id="rtTog"' + (S.on ? ' checked' : '') + '>' +
        '<b>Exclude same-day round trips</b></label>' +
      '<span style="color:#8b97ad">hide every institution + stock + day whose buy and sell quantities agree within</span>' +
      '<select id="rtTol">' + [0.1, 0.5, 1, 2, 5].map(v => '<option value="' + v + '"' + (v === S.tol ? ' selected' : '') + '>' + v + '%</option>').join('') + '</select>' +
      '<span style="color:#8b97ad">' + (S.on
        ? '\u2014 <b>' + fn(S.all.length - DEALS.length) + '</b> of ' + fn(S.all.length) + ' rows hidden'
        : '\u2014 currently off, showing all ' + fn(S.all.length) + ' rows') + '</span>' +
    '</div>' +
    '<div class="tabs">' + TABS.map(t => '<div class="tab' + (t[0] === OPEN_TAB ? ' on' : '') + '" data-p="' + t[0] + '">' + t[1] + '</div>').join('') + '</div>' +
    '<div id="panes">' + TABS.map(t => '<div class="pane' + (t[0] === OPEN_TAB ? '' : ' hide') + '" id="p_' + t[0] + '"></div>').join('') + '</div></div>';

  const rtT = document.getElementById('rtTog'), rtS = document.getElementById('rtTol');
    if (rtT) rtT.onchange = () => { S.on = rtT.checked; rerenderDashboard(); };
    if (rtS) rtS.onchange = () => { S.tol = +rtS.value; if (S.on) rerenderDashboard(); };
    document.querySelectorAll('.tab').forEach(el => el.addEventListener('click', () => { S.tab = el.dataset.p; }));
    setTimeout(() => { if (S.tab) { const t = document.querySelector('.tab[data-p="' + S.tab + '"]'); if (t) t.click(); } }, 0);
  ...
  // ---- Latest day ----
  // The single most recent date the data holds, and nothing else. It follows the
  // data, not the clock, so it does not go blank at midnight or when a scrape is late.
  (function () {
    const rows = orderDeals(rbd[LATEST] || []);
    ...
    const stamp = LATEST ? dfmt(LATEST) + ' ' + LATEST.slice(0, 4) + ' \u00B7 ' + wd(LATEST) : '';
    // Age of the data against the machine's clock - shown, never used to pick the date.
    const clock = (function () { const d = new Date(), p = n => String(n).padStart(2, '0'); return d.getFullYear() + '-' + p(d.getMonth() + 1) + '-' + p(d.getDate()); })();
    const behind = LATEST && LATEST < clock
      ? Math.round((new Date(clock + 'T00:00:00') - new Date(LATEST + 'T00:00:00')) / 86400000) : 0;
    const age = behind
      ? '<div class="cs" style="margin-top:7px">Data is <b>' + behind + ' day' + (behind > 1 ? 's' : '') + '</b> behind this machine\u2019s clock (' +
        dfmt(clock) + ' ' + clock.slice(0, 4) + ') \u2014 the market was shut, or the scrape has not been run since.</div>' : '';

    if (!rows.length) {
      document.getElementById('p_td').innerHTML =
        '<div class="card"><h2>Latest trading day' + (stamp ? ' \u2014 ' + stamp : '') + '</h2>' +
        '<div class="cs">This tab shows one date \u2014 the most recent the data holds \u2014 and nothing else. Use <b>Daily</b> for history.</div>' +
        '<div class="note" style="margin:12px 0 0">' + (LATEST
          ? 'Every deal on <b>' + stamp + '</b> is hidden by the round-trip filter. Turn the toggle off, or widen the tolerance, to see them.'
          : 'This dataset is empty \u2014 there is no trading day to show.') + '</div></div>';
      return;
    }
    ...
    document.getElementById('p_td').innerHTML =
      '<div class="card"><h2>Latest trading day \u2014 ' + stamp + '</h2><div class="cs">Every stock bought and sold on that date, with the client behind each leg and what it was worth. ' +
      'This tab shows one date \u2014 the most recent the data holds \u2014 and nothing else. Use <b>Daily</b> for history.</div>' +
      ...
      'cr \u00B7 sold \u20B9' + fcp(tSV) + 'cr \u00B7 net ' + moneyC(tBV - tSV) + ' \u00B7 turnover \u20B9' + fcp(tBV + tSV) + 'cr</div>' + age + '</div>' +
      side('mTdBuy', 'Bought on ' + dfmt(LATEST), buys, tBV) +
      side('mTdSell', 'Sold on ' + dfmt(LATEST), sells, tSV);

    [['mTdBuy', buys], ['mTdSell', sells]].forEach(([id, list]) => paginatedTable(id, {
      rows: list, sortIdx: 6, pageSize: 250, placeholder: 'Search stock, institution, exchange...',
  ...
  paginatedTable('mStock', { rows: stocks, sortIdx: 9, pageSize: 250, placeholder: 'Search all stocks...', text: r => r.name,
  ...
  paginatedTable('mInst', { rows: insts, sortIdx: 9, pageSize: 250, placeholder: 'Search all institutions...', text: r => r.name,
  ...
  paginatedTable('mGS', { rows: stocks, sortIdx: 7, pageSize: 250, placeholder: 'Search...',
  ...
  paginatedTable('mGI', { rows: insts, sortIdx: 7, pageSize: 250, placeholder: 'Search...',
  ...
  paginatedTable('mDay', {
    rows: days, sortIdx: 0, pageSize: 250, sizes: [10, 25, 50, 250], placeholder: 'Search a date, stock or institution across all days...',
    ...
    expand: r => {
      ...
      setTimeout(() => {
        ...
        m.dataset.done = 1;
        paginatedTable(m, {
          rows: orderDeals(rbd[r.name] || []), pageSize: 250, sizes: [10, 25, 50, 100, 250],
  ...
  paginatedTable('mDayC', {
    rows: XC.days, sortIdx: 0, pageSize: 250, sizes: [10, 25, 50, 250], placeholder: 'Search a date, stock or institution across all days...',
    ...
    expand: r => {
      ...
      setTimeout(() => {
        ...
        m.dataset.done = 1;
        paginatedTable(m, {
          rows: orderDeals(XC.rbd[r.name] || []), pageSize: 250, sizes: [10, 25, 50, 100, 250],
          placeholder: 'Filter this day...', text: x => x.stock + ' ' + x.client + ' ' + x.exch + ' ' + x.type + ' ' + x.action,
          cols: [{ h: 'Stock', l: 1, f: x => '<b>' + x.stock + '</b>', v: x => x.stock },
                 { h: 'Institution / Client', l: 1, f: x => x.client, v: x => x.client }].concat(X.dealCols({ h: '', f: () => '' }).slice(1))
        });
  ...
  paginatedTable('mDeals', {
    rows: orderDeals(DEALS), sortIdx: 0, pageSize: 250, placeholder: 'Search stock, institution, date, exchange...',
